...
class GitClient:
    ...
    def __init__(self, config: Settings, repo_path: Path | None = None):
        self.config = config
        self.repo_path = repo_path if repo_path is not None else config.REPO_PATH
    ...
    def commit(self, message: str, commit_date: date) -> None:
        """创建提交"""
        hour = random.randint(*self.config.HOUR_RANGE)
        minute = random.randint(*self.config.MINUTE_RANGE)
        second = random.randint(*self.config.SECOND_RANGE)
        author_date = f"{commit_date.isoformat()} {hour:02d}:{minute:02d}:{second:02d}"
